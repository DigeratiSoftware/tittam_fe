...
import type { TemplateField, NumberField } from "@/services/template-service"
import { templateService } from "@/services/template-service"

interface CreateTemplateFieldDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  onSubmit: (field: Omit<TemplateField, "id">) => void
}
...
const dataTypeOptions = [
  ...
  { value: "calculation", label: "Calculation" } // Added calculation type
]
...
export function CreateTemplateFieldDialog({ open, onOpenChange, onSubmit }: CreateTemplateFieldDialogProps) {
  const [formData, setFormData] = useState({
    ...
    validation: {
      ...
      field2: "",
      operator: "",
      dateAllowed: "" as "future" | "past" | "both" | "",
    },
  })
  const [newOption, setNewOption] = useState("")
  const [loading, setLoading] = useState(false)
  const [numberFields, setNumberFields] = useState<NumberField[]>([]) // Added state for number fields

  useEffect(() => {
    if (formData.dataType === "calculation") {
      loadNumberFields()
    }
  }, [formData.dataType])

  const loadNumberFields = async () => {
    try {
      const fields = await templateService.getNumberFields()
      setNumberFields(fields)
    } catch (error) {
      console.error("Failed to load number fields:", error)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.dataType === "number" || formData.dataType === "text" || formData.dataType === "textarea") {
        if (formData.validation.min !== undefined) validation.min = formData.validation.min
        if (formData.validation.max !== undefined) validation.max = formData.validation.max
      }

      if (formData.dataType === "calculation") {
        validation.field1 = formData.validation.field1
        validation.field2 = formData.validation.field2
        validation.operator = formData.validation.operator
      }

      if (formData.dataType === "date") {
        validation.dateAllowed = formData.validation.dateAllowed || "both"
      }
      ...
      const fieldData: Omit<TemplateField, "id"> = {
        ...
        isDefault: false,
        order: 999,
        group: "Custom Fields", // Default group for new fields
  ...
  const needsCalculation = formData.dataType === "calculation" // Added calculation check
  const needsDateValidation = formData.dataType === "date" // Added date validation check
        ...
        <div className="relative">
          <DialogHeader className="pb-6">
            ...
            <DialogDescription className="text-slate-600">
              Create a new field for your template with validation rules
            </DialogDescription>
          </DialogHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {needsCalculation && (
              <div className="space-y-3 p-4 bg-slate-50 rounded-lg border border-slate-200">
                <Label className="text-sm font-semibold text-slate-700">Calculation Settings</Label>
                <div className="grid grid-cols-3 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="field1" className="text-xs text-slate-600">
                      Field 1
                    </Label>
                    <Select
                      value={formData.validation.field1}
                      onValueChange={(value) =>
                        setFormData((prev) => ({
                          ...prev,
                          validation: { ...prev.validation, field1: value },
                        }))
                      }
                    >
                      <SelectTrigger className="h-10 bg-white border-slate-300 focus:border-[#013A65]">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="operator" className="text-xs text-slate-600">
                      Operator
                    </Label>
                    <Select
                      value={formData.validation.operator}
                      onValueChange={(value) =>
                        setFormData((prev) => ({
                          ...prev,
                          validation: { ...prev.validation, operator: value },
                        }))
                      }
                    >
                      <SelectTrigger className="h-10 bg-white border-slate-300 focus:border-[#013A65]">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="field2" className="text-xs text-slate-600">
                      Field 2
                    </Label>
                    <Select
                      value={formData.validation.field2}
                      onValueChange={(value) =>
                        setFormData((prev) => ({
                          ...prev,
                          validation: { ...prev.validation, field2: value },
                        }))
                      }
                    >
                      <SelectTrigger className="h-10 bg-white border-slate-300 focus:border-[#013A65]">
            ...
            {needsDateValidation && (
              <div className="space-y-3 p-4 bg-slate-50 rounded-lg border border-slate-200">
                <Label className="text-sm font-semibold text-slate-700">Date Validation</Label>
                <Select
                  value={formData.validation.dateAllowed}
                  onValueChange={(value: "future" | "past" | "both") =>
                    setFormData((prev) => ({
                      ...prev,
                      validation: { ...prev.validation, dateAllowed: value },
                    }))
                  }
                >
                  <SelectTrigger className="h-10 bg-white border-slate-300 focus:border-[#013A65]">
                  ...
                  <SelectContent className="bg-white">
                    ...
                    <SelectItem value="both">Both future and past</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            )}
